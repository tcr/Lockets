      ...
      socket.on('request', function(fileName) {
        ...
        try {
          ...
          fs.stat(filePath, function(err, stats) {
            ...
            // watch the file now
            var prev = stats;
            watcher = fs.watch(filePath, { persistent: false }, function (event) {
              try {
                var curr = fs.statSync(filePath);
              } catch (e) {
                unwatch();
                return;
              }

              if (prev && prev.size > curr.size) {
                prev = curr;
                return;
              }

              streamData(filePath, prev ? prev.size : 0, curr.size, socket);
